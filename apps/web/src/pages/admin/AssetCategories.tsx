...
export default function AssetCategories() {
  ...
  const load = () => {
    setLoading(true);
    api.get('/admin/asset-categories').then(r => setData(r.data.data)).finally(() => setLoading(false));
  };
  useEffect(load, []);
  ...
  const onSave = async () => {
    ...
    if (modal.record) {
      await api.put(`/admin/asset-categories/${modal.record.id}`, values);
    } else {
      await api.post('/admin/asset-categories', values);
    }
  ...
  return (
    <Card title={<Typography.Title level={5} style={{ margin: 0 }}>资产分类管理</Typography.Title>}
      ...
      />
      <Modal title={modal.record ? '编辑分类' : '新增分类'} open={modal.open} onCancel={() => setModal({ open: false })} onOk={onSave}>

...
export default function SoftwareCategories() {
  ...
  const load = () => {
    setLoading(true);
    api.get('/admin/software-categories').then((r) => setData(r.data.data)).finally(() => setLoading(false));
  };
  useEffect(load, []);
  ...
  const onSave = async () => {
    ...
    if (modal.record) {
      await api.put(`/admin/software-categories/${modal.record.id}`, values);
    } else {
      await api.post('/admin/software-categories', values);
    }
  ...
  const onDelete = async (id: number) => {
    await api.delete(`/admin/software-categories/${id}`);
    message.success('已删除');
    load();
  };

  return (
    <Card title={<Typography.Title level={5} style={{ margin: 0 }}>软件分类管理</Typography.Title>}
